...
async function update(context, pullRequest) {
  ...
  if (updateMethod === "merge") {
    ...
    newSha = await api_merge(octokit, updateRetries, updateRetrySleep, pullRequest)
...
async function api_merge(octokit, updateRetries, updateRetrySleep, pullRequest) {
  const mergeableState = await pullRequestState(
    octokit,
    updateRetries,
    updateRetrySleep,
  ...
  if (mergeableState === "clean" || mergeableState === "has_hooks") {
    logger.info("No update necessary, mergeable_state:", mergeableState);
    return pullRequest.head.sha;
  ...
  logger.info(`Merging latest changes from ${baseRef} into ${headRef} using GitHub API`);
  const { status, data } = await octokit.pulls.updateBranch({
    owner: pullRequest.base.repo.owner.login,
    repo: pullRequest.base.repo.name,
    pull_number: pullRequest.number,
    expected_head_sha: baseRef
  });
  ...
  if (status === 204) {
    ...
    logger.info("Merge succeeded, new HEAD:", headRef, baseRef);
    return headRef;
  } else {
    logger.warning("Merge unsuccessful", status, data)
    return null
  }
}
